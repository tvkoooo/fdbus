...
static bool fdb_secure_test = false;
...
public:
    ...
    void sendData()
    {
        ...
        if (fdb_init_skip_count)
        {
            send(XCLT_TEST_SINGLE_DIRECTION, mBuffer, fdb_block_size,
                 fdb_secure_test ? FDB_QOS_SECURE_BEST_EFFORTS : FDB_QOS_BEST_EFFORTS);
            return;
        }
    ...
    void invokeMethod()
    {
        ...
        if (fdb_sync_invoke)
        {
            ...
            one.add_ints(9);
            std::string data;
            one.SerializeToString(&data);
            size_t len = data.size() + MESSAGE_LENGTH;

            memcpy(mmmbuf, &messageid, MESSAGE_LENGTH);
            memcpy(mmmbuf + MESSAGE_LENGTH, data.c_str(), data.size());
            //test lj
            invoke(ref, mmmbuf, (int32_t)len);
            handleReply(ref);
        }
        else
        {
...
protected:
    ...
    void onOnline(const CFdbOnlineInfo &info)
    {
        if (fdb_secure_test)
        {
            if (!checkOnlineChannel(FDB_SEC_SECURE, info))
            {
                return;
            }
        }
        ...
        {
            if (!checkOnlineChannel(FDB_SEC_INSECURE, info))
            {
                return;
            }
        }
        ...
        subscribe(sub_list, 0, info.mQOS);
    }
    ...
    void handleReply(CBaseJob::Ptr &msg_ref)
    {
        ...
        switch (msg->code())
        {
            case XCLT_TEST_BI_DIRECTION:
            {
                ...
                int geta = 0;
                memcpy(&geta, buffer, sizeof(geta));
                printf("client geta:%d \n", geta);
                //test lj
...
int main(int argc, char **argv)
{
    ...
    int32_t sync_invoke = 1;
    int32_t secure_test = 0;
    const struct fdb_option core_options[] = {
        ...
        { FDB_OPTION_BOOLEAN, "secure", 'c', &secure_test},
    ...
    std::cout << "block size: " << fdb_block_size
              ...
              << ", secure: " << (fdb_secure_test ? "true" : "false")
              << std::endl;
    ...
    if (help)
    {
        ...
        std::cout << "    -c: if set, force test on secure channel; otherwise insecure channel will be tested" << std::endl;
        exit(0);
    }

    CFdbContext::enableLogger(false);
    /* start fdbus context thread */
    //FDB_CONTEXT->start(0, 1000);
    FDB_CONTEXT->start();
    ...
    fdb_xtest_client->setOnlineChannelType(FDB_SEC_NO_CHECK);
